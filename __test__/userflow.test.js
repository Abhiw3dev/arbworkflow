...
const cookiesArbLoginPath = './signInCookies/nextArbLoginCookies.json'
...
describe('First User Flow Test', () => {
    ...
    test('Login and Integration Test', async () => {
        ...
        if (fs.existsSync(cookiesArbLoginPath)) {
            const googlepage = await browser.newPage();
            const cookies = JSON.parse(fs.readFileSync( cookiesArbLoginPath, 'utf-8'));
            for (let cookie of cookies) {
                await googlepage.setCookie(cookie);
            }
            console.log('Cookies loaded successfully for Google.');

            await googlepage.goto('https://next.appreviewbot.com/login/');
        } else {
            console.log('No cookies file found for Google.');
            await browser.close()
        }
